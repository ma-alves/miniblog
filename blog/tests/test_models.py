...
class PostModelTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        user = User.objects.create_user(username='TestingUser3', password='#TGRubhs8745')
...
class CommentModelTest(TestCase):
    ...
    def test_comment_author_link_matches_user_link(self):
        post = Comment.objects.get(id=1)
        user = User.objects.get(id=2)
        self.assertEqual(post.comment_author.author.get_absolute_url(), user.author.get_absolute_url())
